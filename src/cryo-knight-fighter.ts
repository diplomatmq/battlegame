...
export class CryoKnightFighter extends Fighter {
	private readonly ambFrost: CryoAmbient[] = [];
	private readonly cryoTrails: CryoTrail[] = [];
	private readonly cryoParts: CryoPart[] = [];
	private frostNova: FrostNova | null = null;
	private frostPoint: FrostPoint | null = null;
	private capePh = Math.random() * Math.PI * 2;
	...
	private advanceCryoVfx(bladeActive: boolean): void {
		this.capePh += 0.08;
		...
		for (let i = this.cryoTrails.length - 1; i >= 0; i--) {
			...
			t.life -= 0.06;
			if (t.life <= 0) this.cryoTrails.splice(i, 1);
		}
		...
		if (this.frostNova) {
			...
			if (this.frostNova.alpha <= 0 || this.frostNova.r > this.frostNova.maxR) this.frostNova = null;
		}
