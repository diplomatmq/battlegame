...
import {
  getCoins, setCoins, SHOP_CATALOGUE, ShopItemDef, ItemSlot,
  getEquipped, setEquipped, getTotalStats, getLevel, getXP, getXPForNextLevel,
  recordCoinSpend,
} from "./player.js";
...
function getOwned(): string[] { return JSON.parse(localStorage.getItem("shopOwned") ?? "[]") as string[]; }
function addOwned(id: string): void {
  const o = getOwned(); if (!o.includes(id)) { o.push(id); localStorage.setItem("shopOwned", JSON.stringify(o)); }
}
...
const statsEl  = document.getElementById("statsPanel")  as HTMLElement | null;
const levelEl  = document.getElementById("playerLevel") as HTMLElement | null;
const xpBarEl  = document.getElementById("xpBar")       as HTMLElement | null;
const xpTextEl = document.getElementById("xpText")      as HTMLElement | null;
...
function refreshHeader(): void {
  ...
  if (levelEl)  levelEl.textContent  = `LVL ${getLevel()}`;
  if (xpBarEl)  xpBarEl.style.width  = `${Math.min(100, Math.round(getXP() / getXPForNextLevel() * 100))}%`;
  if (xpTextEl) xpTextEl.textContent = `${getXP()} / ${getXPForNextLevel()} XP`;
}
refreshHeader();
...
function renderGrid(containerId: string, items: ShopItemDef[]): void {
  ...
  for (const item of items) {
    ...
    const isEquipped = equipped[item.slot as ItemSlot] === item.id;

    const bonusParts: string[] = [];
    if (item.bonus.atk) bonusParts.push(`\u2694 +${item.bonus.atk} \u0410\u0422\u041a`);
    if (item.bonus.def) bonusParts.push(`\uD83D\uDEE1 +${item.bonus.def} \u0414\u0415\u0424`);
    if (item.bonus.spd) bonusParts.push(`\u26A1 +${item.bonus.spd} \u0421\u041f\u0414`);
    ...
    if (!isOwned) div.addEventListener("click", () => buyItem(item));

    const btn = div.querySelector<HTMLButtonElement>(".equip-btn");
    btn?.addEventListener("click", (e) => { e.stopPropagation(); toggleEquip(item.id, item.slot as ItemSlot); });

    container.appendChild(div);
  }
}

function buyItem(item: ShopItemDef): void {
  const coins = getCoins();
  if (coins < item.price) { showToast("\u041d\u0435\u0434\u043e\u0441\u0442\u0430\u0442\u043e\u0447\u043d\u043e \u043c\u043e\u043d\u0435\u0442!"); return; }
...
function toggleEquip(id: string, slot: ItemSlot): void {
  const eq = getEquipped();
  if (eq[slot] === id) { eq[slot] = null; showToast("\u0421\u043d\u044f\u0442\u043e \u0441 \u043f\u0435\u0440\u0441\u043e\u043d\u0430\u0436\u0430"); }
  else { eq[slot] = id; showToast(`\u042d\u043a\u0438\u043f\u0438\u0440\u043e\u0432\u0430\u043d\u043e \u0432 \u0431\u043e\u044e!`); }
  setEquipped(eq);
  refreshHeader();
  renderAll();
}
...
(function startBg() {
  ...
  const pts: { x: number; y: number; s: number; v: number }[] = [];
  for (let i = 0; i < 80; i++) pts.push({ x: Math.random()*cvs.width, y: Math.random()*cvs.height, s: Math.random()*1.5+0.4, v: Math.random()*0.3+0.08 });
  (function tick() {
    ctx.fillStyle = "#0a0308"; ctx.fillRect(0, 0, cvs.width, cvs.height);
    for (const p of pts) {
      ctx.fillStyle = `rgba(255,180,80,${0.2+Math.random()*0.2})`; ctx.fillRect(p.x, p.y, p.s, p.s);
      p.y += p.v; if (p.y > cvs.height) { p.y = 0; p.x = Math.random()*cvs.width; }
    }
