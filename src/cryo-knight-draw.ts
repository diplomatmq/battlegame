...
export function drawCryoKnight(
  ...
  if (fighterState === "victory") {
    ...
    c.strokeStyle = "rgba(74,200,232,.5)"; c.lineWidth = 1.4;
    c.shadowColor = "#4ac8e8"; c.shadowBlur = 12;
    c.beginPath(); c.ellipse(30, 1, 18, 4.5, 0, 0, Math.PI * 2); c.stroke();
    c.restore();
  }
  c.restore();

  // 3. Global VFX
  if (frostNova) _drawFrostNova(c, frostNova, scale);
  _drawTrails(c, cryoTrails);
  _drawDP(c, cryoParts);
  if (frostPoint) _drawFrostPoint(c, frostPoint);
}

function _shadow(c: CanvasRenderingContext2D, x: number, y: number, thrustX: number, jumpY: number, scale: number, state: string) {
  c.save();
  let alpha = (state === "death") ? 0.1 : 0.22;
  if (jumpY < 0) alpha *= Math.max(0.08, 1 - Math.abs(jumpY) / (105 * scale) * 0.8);
  c.globalAlpha = alpha;
  c.fillStyle = "#000"; c.beginPath(); c.ellipse(x + thrustX, y + 2, 28 * scale, 5 * scale, 0, 0, 6.283); c.fill();
  c.restore();
}

function _cape(c: CanvasRenderingContext2D, t: number) {
  c.save();
...
function _backArm(c: CanvasRenderingContext2D) {
  c.save();
...
function _legs(c: CanvasRenderingContext2D) {
  c.save();
...
function _torso(c: CanvasRenderingContext2D, eyeGlow: number) {
  c.save();
  ...
  c.save(); c.shadowColor = "#4ac8e8"; c.shadowBlur = 4 * eyeGlow;
  ...
  c.save(); c.strokeStyle = `rgba(55,155,195,${0.11 * eyeGlow})`; c.lineWidth = 0.5;
  c.beginPath(); c.moveTo(-7, -82); c.lineTo(0, -74); c.lineTo(7, -82); c.stroke(); c.restore();
  c.restore();
}

function _head(c: CanvasRenderingContext2D, eyeGlow: number) {
  c.save();
  ...
  c.save(); c.shadowColor = "#4ac8e8"; c.shadowBlur = 12 * eyeGlow;
  c.fillStyle = `rgba(74,200,232,${0.85 * eyeGlow})`;
  c.beginPath(); c.ellipse(-5, -109, 3.5, 1.5, 0, 0, 6.283); c.fill();
  c.beginPath(); c.ellipse(5, -109, 3.5, 1.5, 0, 0, 6.283); c.fill();
  c.shadowBlur = 5 * eyeGlow; c.fillStyle = `rgba(200,245,255,${0.65 * eyeGlow})`;
...
function _shield(c: CanvasRenderingContext2D, eyeGlow: number) {
  ...
  c.save(); c.shadowColor = "#4ac8e8"; c.shadowBlur = 5 * eyeGlow;
  c.strokeStyle = `rgba(65,175,215,${0.18 * eyeGlow})`; c.lineWidth = 0.55; c.translate(0, 2);
  for (let i = 0; i < 3; i++) {
    const a = i * Math.PI / 3;
  ...
  c.restore();
  c.fillStyle = `rgba(65,175,215,${0.12 * eyeGlow})`; c.beginPath(); c.arc(0, 2, 1.8, 0, 6.283); c.fill();
  c.restore();
}

function _swordArm(c: CanvasRenderingContext2D, armAng: number, eyeGlow: number, ambFrost: any[], state: string) {
  c.save(); c.translate(20, -88); c.rotate(armAng);
  ...
  arm(2, 17, "#0b0b1d");
  c.beginPath(); c.ellipse(0, 17, 4.5, 3, 0, 0, 6.283); c.fill(); c.stroke();
  arm(17, 32, "#0d0d20");
  ...
  _sword(c, 38, eyeGlow);
  if (state !== "death") _ambFrostInner(c, 38, eyeGlow, ambFrost);
  c.restore();
}

function _sword(c: CanvasRenderingContext2D, hy: number, eyeGlow: number) {
  ...
  bg.addColorStop(0, "rgba(48,128,168,.42)"); bg.addColorStop(.5, "rgba(28,68,88,.68)"); bg.addColorStop(1, "rgba(48,128,168,.42)");
  c.fillStyle = bg; c.fill(); c.strokeStyle = "rgba(95,205,242,.32)"; c.lineWidth = 0.55; c.stroke(); c.restore();
  c.restore();
}

function _ambFrostInner(c: CanvasRenderingContext2D, hy: number, eyeGlow: number, ambFrost: any[]) {
  c.save(); c.translate(0, hy + 25);
  for (const p of ambFrost) {
    const px = Math.cos(p.a) * p.d, py = Math.sin(p.a) * p.d + (p.off - 25);
    c.globalAlpha = p.op * Math.max(0.15, eyeGlow); c.fillStyle = "#80d8f0";
    c.shadowColor = "#4ac8e8"; c.shadowBlur = 3; c.beginPath(); c.arc(px, py, p.sz, 0, 6.283); c.fill()
  }
  c.restore();
}

function _drawTrails(c: CanvasRenderingContext2D, cryoTrails: any[]) {
...
function _drawDP(c: CanvasRenderingContext2D, cryoParts: any[]) {
...
function _drawFrostPoint(c: CanvasRenderingContext2D, fp: any) {
  if (!fp || fp.life <= 0) return;
  c.save();
  ...
  c.fillStyle = gr; c.beginPath(); c.arc(fp.x, fp.y, r, 0, Math.PI * 2); c.fill();
  c.restore();
}
function _drawFrostNova(c: CanvasRenderingContext2D, n: any, scale: number) {
