...
function drawRuneCircle(ctx, x, y, r, rotation, alpha) {
    ctx.save();
    ctx.translate(x, y);
    ctx.rotate(rotation);
...
function drawBattleMageFigure(ctx, opts) {
...
export class JadeMageFighter extends Fighter {
    ...
    releaseAttack(type) {
        ...
        if (type === "normal") {
            ...
            for (let i = 0; i < 8; i++)
                this.particles.push(new Particle(hand.x, hand.y, JADE.primary, 1.35));
            return;
        }
        ...
        for (let i = 0; i < 20; i++)
            this.particles.push(new Particle(staff.x, staff.y, JADE.bright, 2.25));
    }
    startSkyBeam(damage) {
        ...
        for (let i = 0; i < 22; i++)
            this.particles.push(new Particle(targetX, 62 + Math.random() * 18, JADE.cold, 2.0));
    }
    updateSkyBeam() {
    ...
    updateProjectiles() {
        ...
        for (let i = this.projectiles.length - 1; i >= 0; i--) {
            ...
            const hit = Math.abs(p.x - this.opponent.x) < (30 + p.radius + hitPad) &&
