...
function renderChallenge(ch) {
    ...
    if (done && !claimed) {
        const btn = card.querySelector(`#claim_${ch.id}`);
...
(function startBg() {
    ...
    const pts = [];
    for (let i = 0; i < 90; i++)
        pts.push({ x: Math.random() * cvs.width, y: Math.random() * cvs.height, s: Math.random() * 2 + 0.3, v: Math.random() * 0.2 + 0.06, a: Math.random() * 0.3 + 0.1 });
    (function tick() {
